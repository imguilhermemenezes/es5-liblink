...
export default function ProtectedLayout() {
    ...
    return (
        <div className="app-container">
            ...
            {/* Mobile Top Header (somente visível no celular) */}
            <header className="mobile-header">
                ...
                <div className="mobile-logo">Liblink</div>
            </header>
            ...
            <aside className={`sidebar ${isSidebarOpen ? 'open' : ''}`}>
                ...
                <nav className="sidebar-nav">
                    <Link to="/dashboard" onClick={closeSidebar} className={`nav-item ${isActive('/dashboard')}`}>
                        <Home size={20} /> Dashboard
                    </Link>
                    <Link to="/books" onClick={closeSidebar} className={`nav-item ${isActive('/books')}`}>
                        <BookOpen size={20} /> Acervo
                    </Link>
                    <Link to="/loans" onClick={closeSidebar} className={`nav-item ${isActive('/loans')}`}>
                        <BookDown size={20} /> Empréstimos
                    </Link>

                    {user.role === 'admin' && (
                        <>
                            <Link to="/team" onClick={closeSidebar} className={`nav-item ${isActive('/team')}`}>
                                <Users size={20} /> Bibliotecários
                            </Link>
                            <Link to="/settings" onClick={closeSidebar} className={`nav-item ${isActive('/settings')}`}>
